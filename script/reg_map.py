...
* @Description  : {sheet_name} reg map cfg
...
'''
        ...
        self.signal_rstn      = 'rst_n_i'       if self.name_rule else 'rst_n_i'
        self.signal_clk       = 'sys_clk_i'     if self.name_rule else 'sys_clk_i'
        self.signal_p_sel     = 'ram_rd_addr_i' if self.name_rule else 'ram_rd_addr_i'
        self.signal_p_enbale  = 'ram_rd_en_i'   if self.name_rule else 'ram_rd_en_i'
        self.signal_p_addr    = 'ram_wr_addr_i' if self.name_rule else 'ram_wr_addr_i'
        self.signal_p_write   = 'ram_wr_en_i'   if self.name_rule else 'ram_wr_en_i'
        self.signal_p_wr_data = 'ram_wr_data_i' if self.name_rule else 'ram_wr_data_i'
        self.signal_p_rd_data = 'ram_rd_data_o' if self.name_rule else 'ram_rd_data_o'
    ...
    def cal_max_len(self,df=None):
        ...
        if ( rd_en_reg or wr_en_reg ):
            self.max_len_of_wire_declare_right_val = max(11+len(self.signal_p_sel)+len(self.signal_p_write)+len(self.signal_p_enbale),18+len(self.signal_p_addr)+self.max_len_of_RegName)# wire spi_wr_data_rd = ( ram_wr_addr_i == SPI_WR_DATA ) && ram_rd_en;
    ...
    def gen_module_head(self,sheet_name=None):
        self.port_lst.append(f'''module {sheet_name}{"_APB_CFG" if sheet_name.isupper() else "_reg_map_cfg"}''')
        ...
        self.port_lst.append(f'''{sheet.tab}{f'{"input":<{self.max_len_of_extend_port_direction}}'+" [32-1:0]":<{self.max_len_of_port_width}} {self.signal_p_sel:<{self.max_len_of_port_name+self.max_len_of_name_suffix}},''')
        ...
        self.control_lst.append(f'''{"wire":<4} {'ram_wr_en':<{self.max_len_of_RegName+3}} = {self.signal_p_write:<{self.max_len_of_wire_declare_right_val}};''')
        self.control_lst.append(f'''{"wire":<4} {'ram_rd_en':<{self.max_len_of_RegName+3}} = {self.signal_p_enbale:<{self.max_len_of_wire_declare_right_val}};''')
    ...
    def gen_block_rd(self):
        ...
        if( block_rd_lst ):
            ...
            self.block_lst.append(f'''{sheet.tab}case( {self.signal_p_sel} )''')
            self.block_lst.extend(block_rd_lst)
            self.block_lst.append(f'''{sheet.tab*2}default:''')
            self.block_lst.append(f'''{sheet.tab*3}{self.signal_p_rd_data} <= 'h5555_AAAA;''')
    ...
    def process_Reg(self,group=None,BaseAddress=None,OffsetAddress=None,RegName=None):
        ...
        if ( group_control_dict['rd_en'] ):
            ...
            if group_rd_ctrl_en:
                self.control_lst.append(f'''{"wire":<4} {RegName.lower()+'_rd':<{self.max_len_of_RegName+3}} = {'( '+self.signal_p_addr+' == '+f'{RegName.upper():<{self.max_len_of_RegName}}'+' ) && ram_rd_en':<{self.max_len_of_wire_declare_right_val}};''')
        if ( group_control_dict['wr_en'] or group_control_dict['rd_en_wr'] ):
            self.control_lst.append(f'''{"wire":<4} {RegName.lower()+'_wr':<{self.max_len_of_RegName+3}} = {'( '+self.signal_p_addr+' == '+f'{RegName.upper():<{self.max_len_of_RegName}}'+' ) && ram_wr_en':<{self.max_len_of_wire_declare_right_val}};''')
        ...
        if ( group_entire_en ):
            if ( group_control_dict['wr_en'] ):
                if ( group_control_dict['wr_once'] ):
                    ...
                    block_block_lst.append(f'always @ ( posedge {self.signal_clk } )')
        ...
        for row_index in group.index:
            ...
            if ( control_dict['wr_en'] ):
                self.port_lst.append(f'''{sheet.tab}{"output "+("" if group_entire_en else "reg"):<{self.max_len_of_extend_port_direction}} {'['+f'{str(group.loc[row_index,"Len_Cal"]):>{self.max_len_of_Width}}'+"-1:0]" if group.loc[row_index,"Len_Cal"] > 1 else ' ' :>{self.max_len_of_port_width}} {group.loc[row_index,"FieldName_Cal"]:<{self.max_len_of_port_name+self.max_len_of_name_suffix}},//output {RegName} {group.loc[row_index,"FieldName_Cal"]} [{str(group.loc[row_index,"Len_Cal"]):>{self.max_len_of_Width}}-1:0]''')
            ...
            if ( control_dict['rd_en'] ):
                if( group.loc[row_index,"FieldName"].lower() == "reserved" ):
                    ...
                    if not( control_dict['wr_en'] ):
                        self.port_lst.append(f'''{sheet.tab}{"input":<{self.max_len_of_extend_port_direction}} {'['+f'{str(group.loc[row_index,"Len_Cal"]):>{self.max_len_of_Width}}'+"-1:0]" if group.loc[row_index,"Len_Cal"] > 1 else ' ' :>{self.max_len_of_port_width}} {group.loc[row_index,"FieldName_Cal"]:<{self.max_len_of_port_name+self.max_len_of_name_suffix}},//input {RegName} {group.loc[row_index,"FieldName_Cal"]} [{str(group.loc[row_index,"Len_Cal"]):>{self.max_len_of_Width}}-1:0]''')
    ...
    def gen_file(self,sheet_name='',tar_file_name=''):
        ...
        last_port = self.port_lst[-1]
        slash_index = last_port.find('//')
        if slash_index != -1:
            part1 = last_port[:slash_index]
            part2 = last_port[slash_index:]

            part1 = part1.rstrip(',')
            self.port_lst[-1] = part1 + " " + part2
        self.port_lst.append(f');')
        self.block_lst.append(f'\nendmodule')
